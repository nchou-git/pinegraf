...
Pinegraf uses one model for extraction and LLM entity disambiguation. The default is `gpt-5.5`.
...
To change models, set `EXTRACTION_MODEL` on both the `pinegraf-parse` Cloud Run Job and the `pinegraf` service. If unset, both default to `gpt-5.5`.
