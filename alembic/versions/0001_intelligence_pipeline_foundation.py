...
def _create_tables() -> None:
    ...
    op.create_table(
        ...
        sa.Column("primary_attributes", postgresql.JSONB()),
    ...
    op.create_table(
        ...
        sa.Column("predicates", postgresql.ARRAY(sa.Text()), nullable=False),
...
def _create_indexes() -> None:
    ...
    op.execute("CREATE INDEX ix_source_runs_started_at_desc ON source_runs (started_at DESC)")

    _create_index("ix_fetches_source_run_id", "fetches", ["source_run_id"])
    _create_index("ix_fetches_url", "fetches", ["url"])
    op.execute("CREATE INDEX ix_fetches_fetched_at_desc ON fetches (fetched_at DESC)")
    op.execute(
    ...
    op.execute(
        "CREATE INDEX ix_entity_aliases_alias_trgm ON entity_aliases USING gin (alias gin_trgm_ops)"
    )
