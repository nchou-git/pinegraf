...
const panels = { lookup: document.getElementById("panel-lookup"), research: document.getElementById("panel-research") };
tabs.forEach((tab) => tab.addEventListener("click", () => {
  tabs.forEach((t) => t.classList.toggle("active", t === tab));
  Object.entries(panels).forEach(([n, el]) => { el.hidden = n !== tab.dataset.tab; });
}));
...
function renderResults(data) {
  lkResults.innerHTML = "";
  if (!data.results || data.results.length === 0) { lkResults.innerHTML = '<div class="empty">No matches.</div>'; return; }
  ...
  for (const p of data.results) {
    ...
    row.innerHTML = `
      <div class="name">${esc(p.name)} ${p.class_year ? `<span class="meta">${esc(p.class_year)}</span>` : ""}</div>
...
async function runLookup() {
  ...
  try {
    const res = await fetch("/lookup", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    renderResults(await res.json());
  } catch (err) { lkResults.innerHTML = `<div class="empty">Lookup failed: ${esc(err.message)}</div>`; }
}

document.getElementById("lk-go").addEventListener("click", runLookup);
["lk-name", "lk-company", "lk-year"].forEach((id) => {
  document.getElementById(id).addEventListener("keydown", (e) => { if (e.key === "Enter") runLookup(); });
});

const answerEl = document.getElementById("answer");
function selectedMode() {
  const c = document.querySelector('input[name="researchMode"]:checked');
  return c ? c.value : "deep";
}
...
async function runResearch() {
  ...
  try {
    const res = await fetch("/research", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question, mode: "deep" }) });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    answerEl.textContent = data.answer;
  } catch (err) { answerEl.textContent = `Research failed: ${err.message}`; }
}

document.getElementById("rs-go").addEventListener("click", runResearch);
document.getElementById("rs-question").addEventListener("keydown", (e) => { if (e.key === "Enter") runResearch(); });
